...
def main():
    with tempfile.TemporaryDirectory() as temp,sync_playwright() as pw:
        ...
        choice=page.get_by_role('checkbox',name='Allow implicit invocation for deploy-helper',exact=True)
        expect(choice).not_to_be_checked();choice.check()
        expect(page.get_by_role('button',name='Save & apply',exact=True)).to_be_enabled()
        page.reload();page.wait_for_load_state('networkidle');expect(choice).to_be_checked()
        ...
        group=page.get_by_role('checkbox',name='Allow implicit invocation for Personal skills in Qwen Code',exact=True)
        group.check();expect(page.get_by_role('checkbox',name='Allow implicit invocation for review-helper',exact=True)).to_be_checked()
        page.get_by_role('button',name='Discard',exact=True).click();expect(page.get_by_role('checkbox',name='Allow implicit invocation for review-helper',exact=True)).not_to_be_checked()
